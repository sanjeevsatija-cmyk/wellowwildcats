    ...
    <section className="relative min-h-[90vh] bg-green-deep overflow-hidden flex items-center">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute w-20 h-full"
          style={{ right: 300, background: "linear-gradient(180deg,rgba(160,120,50,.07) 0%,rgba(160,120,50,.14) 50%,rgba(160,120,50,.07) 100%)", transform: "skewX(-3deg)" }} />
      </div>
      <div className="grain-overlay" />

      <div className="relative z-10 w-full max-w-[1240px] mx-auto px-14 py-20 grid grid-cols-1 lg:grid-cols-[1fr_440px] gap-16 items-center">
        ...
        <div>
          ...
          <p className="text-[17px] text-white/65 font-light leading-[1.75] max-w-[520px] mb-9">
        ...
        <div>
          <div className="grid grid-cols-2 gap-px bg-gold/[0.18] rounded-lg overflow-hidden">
            {[
              ...
              <div key={label}
                className="bg-green-deep/[0.88] backdrop-blur-md px-5 py-6 text-center transition-colors duration-200 hover:bg-green-mid/60">
                <div className="font-serif text-[44px] font-black text-gold leading-none mb-1">{num}</div>
                <div className="font-condensed text-[11px] font-semibold tracking-[0.12em] uppercase text-white/55">{label}</div>
              </div>
            ))}
            <div className="col-span-2 bg-gold/[0.1] px-5 py-4 text-center">
              <div className="font-serif text-xl font-black text-white">🏆 &nbsp; Multiple Premierships</div>
              <div className="font-condensed text-[11px] font-semibold tracking-[0.12em] uppercase text-white/55 mt-1">
          ...
          {/* Live results panel */}
          <div className="mt-3.5 bg-green-deep/70 backdrop-blur-md border border-gold/[0.18] rounded-lg p-5">
            <div className="flex justify-between items-center mb-3.5">
              <span className="font-condensed text-[11px] font-bold tracking-[0.15em] uppercase text-gold">
                Weekend Results
              </span>
              <span className="flex items-center gap-1.5 font-condensed text-[10px] font-bold tracking-[0.1em] uppercase text-green-400">
            ...
            {[
              ...
              <div key={row.grade} className="flex items-center gap-2 py-2 border-b border-white/[0.06] last:border-0 last:pb-0">
                <span className="font-condensed text-[10px] font-bold tracking-[0.1em] uppercase text-gold w-20 flex-shrink-0">
                  {row.grade}
                </span>
                <span className="text-[13px] text-white/80 flex-1">vs {row.opp}</span>
                <span className="font-condensed text-xs font-bold text-white/60">{row.us} — {row.them}</span>
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black font-condensed flex-shrink-0 ${
                  row.r === "W"
      ...
      {/* Scroll indicator */}
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center gap-2 text-white/30 font-condensed text-[10px] font-semibold tracking-[0.15em] uppercase">
        <div className="w-px h-10 bg-gradient-to-b from-white/30 to-transparent animate-scroll-pulse" />
        Scroll
      </div>
    </section>
  );
